...
import { FiSearch, FiX, FiDownload, FiPlus } from "react-icons/fi";

const statusOptions = [
  { label: "All", value: "all", color: "bg-gray-300 text-gray-800" },
  ...
  { label: "Not Visited", value: "not-visited", color: "bg-red-100 text-red-700" },
];
  ...
  return (
    <aside
      ...
      className="
        ...
        w-full md:w-[360px]
      "
      style={{ minWidth: '220px', maxWidth: '600px', userSelect: isResizing.current ? 'none' : 'auto' }}
    ...
    >
      ...
      <div className="flex flex-wrap gap-3">
        <Badge variant="default" className="bg-gray-200 text-gray-700 px-3 py-1 rounded-xl shadow-sm">
          Total: {stats.total}
        </Badge>
        <Badge className="bg-green-100 text-green-700 px-3 py-1 rounded-xl shadow-sm">
          Visited: {stats.visited}
        </Badge>
        <Badge className="bg-yellow-100 text-yellow-700 px-3 py-1 rounded-xl shadow-sm">
          Planned: {stats.planned}
        </Badge>
        <Badge className="bg-red-100 text-red-700 px-3 py-1 rounded-xl shadow-sm">
      ...
      <div className="flex flex-wrap gap-3" role="group" aria-label="Filter villages by status">
        {statusOptions.map(opt => (
          <button
            key={opt.value}
            className={`px-4 py-1 rounded-full text-sm font-semibold transition select-none focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-blue-500 ${
              filter === opt.value
